...
declaration.builder = (yargs) => yargs
  ...
  .option('update', {
    alias: 'u',
    describe: 'On conflict, updates existing entries if true, ignores new data if false, fails on conflict if set',
    type: 'boolean'
  })
  .option('progress', {
    alias: 'p',
...
declaration.handler = (argv, collection) => {
  ...
  const writer = new SqlStreamWriter(argv.dialect, collection.name, argv.delete, argv.update)
